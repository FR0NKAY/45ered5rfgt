...
void loadconfig()
{
	...
	if (curl)
	{
		curl_easy_setopt(curl, CURLOPT_URL, xorstr("https://monkeyclicker.xyz/users/load_config/"));
		...
		ssdata << usr_param << curl_easy_escape(curl, username.c_str(), strlen(username.c_str())) << pass_param <<
			curl_easy_escape(curl, password.c_str(), strlen(password.c_str()));
		...
		else
		{
			...
			if (response_code == 200)
			{
				...
				if (response != (char*)xorstr("{}\n") && response != (char*)xorstr("") && response !=(char *)xorstr("null"))
				{
...
void saveconfig(bool deleteConfig)
{
	...
	if (curl)
	{
		curl_easy_setopt(curl, CURLOPT_URL, xorstr("https://monkeyclicker.xyz/users/save_config/"));
		std::stringstream ssdata;
		std::string usr_param = (char*)xorstr("usr=");
		std::string pass_param = (char*)xorstr("&pass=");
...
int ClickThread2() // thread dos clicks
{
	...
	while (imguicontrole)
	{
		if (login_button_pressed) {
			...
			//login
			std::string url = xorstr("https://monkeyclicker.xyz/users/app_auth/");
...
int main(char* argv[])
{
	...
	concatenateHwid(xorstr(L"SELECT SerialNumber FROM Win32_DiskDrive "), xorstr(L"SerialNumber"));
	...
	while (imguicontrole)
	{
		...
		ImGui::Begin(xorstr("MonkeyClicker 2.9.5"), nullptr,
		...
		else
		{
			...
			if (imguicontrole)
			{
				...
				if (!login_button_pressed) {
					...
					{
						ShellExecute(0, 0, "https://monkeyclicker.xyz", 0, 0, SW_SHOW);
