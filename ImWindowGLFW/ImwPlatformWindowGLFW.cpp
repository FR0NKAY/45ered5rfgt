...
extern bool loggedinglfw;
extern bool hidewindowtoggle;
extern bool imguicontrole;
int controlehidewindow;
...
bool ImwPlatformWindowGLFW::Init(ImwPlatformWindow* pMain)
{
	...
	m_pWindow = glfwCreateWindow(300, 230, "", NULL, pMainWindow);
...
void ImwPlatformWindowGLFW::PreUpdate()
{
	...
	if (hidewindowtoggle && controlehidewindow == 1)
	{
		
			glfwHideWindow(pMainWindow);
			controlehidewindow = 0;
		

	}
	else
	{
		if (controlehidewindow == 1)
		{
			glfwShowWindow(pMainWindow);
			controlehidewindow = 0;
		}
	...
	if (loggedinglfw)
	{
		glfwSetWindowSize(pMainWindow, 420, 510);
	}
	else
	{
		glfwSetWindowSize(pMainWindow, 300, 230);
	}
